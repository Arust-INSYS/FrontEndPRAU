...
export class CalificacionService {
  ...
  eliminarcriterios(codCalificacion: string): Observable<object> {
    ...
    return this.http.delete(`${this.url}/delete?codCalificacion=${codCalificacion}`, { headers });
}
 
actualizarcriterios(codCalificacion: string, calificacion: Calificacion): Observable<Calificacion> {
  const headers = new HttpHeaders({
    Authorization: `Bearer ${this.localStorage.getItem('token')}`
  });

  // Aquí agregamos el código de calificación a la URL
  return this.http.put<Calificacion>(`${this.url}/update?codCalificacion=${codCalificacion}`, calificacion, { headers });
}
